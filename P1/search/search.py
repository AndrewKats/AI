...
def genericSearch(problem, queue):
	"""
	A generic search function that uses the given queue type
	"""
	...
	while not queue.isEmpty():
		# Depends on which type of queue
		path = queue.pop()

		# The last element of the path is the current state
		state = getCurrentState(path)

		# Goal found
		if problem.isGoalState(state):
			# Return the path
			return getSolutionPath(path)

		# Only follow path if not visited
		if state not in visited:
			# Mark current state visited
			visited.append(state)

			# Go through all the current state's possible successors
			for s in problem.getSuccessors(state):
				# Put unvisited states in queue
